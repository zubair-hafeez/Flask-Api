...
from api_demo.flask_04_jwt_auth_app import app, jwt  # Make sure to import your actual app
...
class JWTTestCase(unittest.TestCase):
    ...
    def test_login_valid_user(self):
        ...
        response = self.app.post('/login', headers={"Content-Type": "application/json"}, data=login_data)
        self.assertEqual(response.status_code, 200)
    ...
    def test_access_protected_route(self):
        ...
        response = self.app.get('/items', headers={"Authorization": f"Bearer {access_token}"})
        self.assertEqual(response.status_code, 200)
